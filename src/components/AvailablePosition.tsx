...
export default function AvailablePosition({position}: {position: Position}){
    return(
        <Link href={`/career/${position.id}`}>
            <div className="bg-white rounded-lg *:flex *:gap-10 p-10 space-y-5">
                <div>
                    <h3 className="w-1/3 text-2xl font-extrabold hover:text-[#21C963]">{position.title}</h3>
                    <div className="w-2/3 flex justify-between items-center">
                        <p className="font-bold">Due to {position.dueDate}</p>
                        <ButtonComponent className="flex gap-3 items-center w-fit hover:bg-stone-500" bg="#2A2A2D" textColour="white">
                            <span>Apply</span>
                ...
                <div>
                    <div className="w-1/3 flex flex-wrap gap-5 text-sm">
                        {position.keywords?.map((item) => (
                            <div key={item} className="bg-[#F4F4F5] p-2 rounded-md">{item}</div>
                        ))}
                    </div>
                    <div className="w-2/3">
