...
export default function AboutSection(){
    return(
        <div className="bg-white pb-10 md:p-20">
            <motion.div initial={{x:-200, opacity: 0}} whileInView={{x:0, opacity: 1}} transition={{duration: .8}} className="flex flex-col w-full items-center mx-auto">
                <Image src="/about-container.png" alt="Cafe" width={1000} height={1000} className="w-full md:rounded-t-4xl m-5 md:m-0 object-cover" />
                <div style={{background: "radial-gradient(ellipse at bottom, #08311A, #000)"}}  className="relative pb-10 bg-[#2A2A2D] w-full md:rounded-b-4xl"> 
                    <div className="text-center flex flex-col md:flex-row items-center justify-between p-10 bg-[url('/grid.png')] bg-cover">
                        <div className="text-center md:text-start space-y-15">
                            <motion.h2 initial={{x:-50, opacity:0}} whileInView={{x:0,opacity:1}} transition={{duration:.9}} className="text-2xl md:text-3xl font-extrabold">Why do we exist?</motion.h2>
                            <motion.div initial={{opacity:0, scaleY:1.5}} whileInView={{opacity:1, scaleY:1}} transition={{duration:.9}} className="bg-gradient-to-b from-[#28DC6E66]/70 to-[#28DC6E66]/40 rounded-2xl p-5 flex gap-5 items-center *:space-y-5">
                                <ul className="space-y-2">
                                    <li className="flex flex-col md:flex-row justify-between md:items-baseline gap-7">
                                        <span className="text-xl font-extrabold text-center md:text-start">$182M</span>
                                        <span className="text-sm text-center md:text-end">In Processing Volume</span>
                                    </li>
                                    <li className="flex flex-col md:flex-row justify-between md:items-baseline gap-7">
                                        <span className="text-xl font-extrabold text-center md:text-start">1,000+</span>
                                        <span className="text-sm text-center md:text-end">Merchants under management</span>
                                    </li>
                                    <li className="flex flex-col md:flex-row justify-between md:items-baseline gap-7">
                                        <span className="text-xl font-extrabold text-center md:text-start">12+</span>
                                        <span className="text-sm text-center md:text-end">Global team members</span>
                                    </li>
                                </ul>
                            </motion.div>
                        </div>
